...
class ova_menu extends Widget_Base {
	...
	protected function register_controls() {
		...
		$this->start_controls_section(
			...
			$this->add_control(
				'submenu_background_color_active',
				[
					'label' => __( 'Background Color Active', 'ova-framework' ),
					'type' => Controls_Manager::COLOR,
					'default' => '',
					'selectors' => [
						'{{WRAPPER}} .ova_nav ul.menu .dropdown-menu li.active > a' => 'background-color: {{VALUE}};',
						'{{WRAPPER}} .ova_nav ul.menu .dropdown-menu li.active > a:after' => 'background-color: {{VALUE}};',
					]
				]
			);
		...
		$this->start_controls_section(
			...
			$this->add_control(
				'shortcode',
				[
					'label' 	=> esc_html__( 'Shortcode ( below Menu )', 'ova-framework' ),
					'type' 		=> Controls_Manager::TEXTAREA,
					'dynamic' => [
						'active' => true,
					],
					'placeholder' => '[egovt-elementor-template id="123"]',
					'description' => esc_html__( 'You can create in Templates ( Elementor )', 'ova-framework' ),
					'default' => '',
				]
			);
	...
	protected function render() {
		...
		$canvas_dir = isset( $settings['canvas_dir'] ) ? $settings['canvas_dir'] : '';
		$canvas_bg = isset( $settings['canvas_bg'] ) ? $settings['canvas_bg'] : '';

		$menu_slug = isset( $settings['menu_slug'] ) ? $settings['menu_slug'] : '';
		$sub_menu_dir = isset( $settings['sub_menu_dir'] ) ? $settings['sub_menu_dir'] : '';
	...
	?>
		...
		<div class="ova_menu_clasic">
			<div class="ova_wrap_nav <?php echo esc_attr( $menu_type ).' '.esc_attr( $openNavBtn_align ).' '.esc_attr( $openNavBtn_align_mobile ); ?>">
					...
					<div class="ova_nav <?php echo esc_attr( $canvas_dir ).' '.esc_attr( $canvas_bg ); ?>">
						...
						<?php
							...
							$walker 	 = new \Ova_Walker_Menu();
						 	...
						 	wp_nav_menu( array(
			                    ...
			                    'container_id'      => '',
			                    'menu_class'        => 'menu'.' '.$sub_menu_dir,
			                    'fallback_cb' => $fallback_cb,
	        					'walker' => $walker
			            	));
			            ?>
			            <div class="shortcode"><?php echo $shortcode; ?></div>
					</div>
